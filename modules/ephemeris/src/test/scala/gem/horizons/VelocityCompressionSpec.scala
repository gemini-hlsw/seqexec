...
final class VelocityCompressionSpec extends CatsSuite with EphemerisTestSupport {
  ...
  test("2014 UR compresses to 99% of original size", RequiresNetwork, Slow) {
    val start = LocalDate.of(2015, Month.SEPTEMBER, 15)
    val end   = LocalDate.of(2015, Month.NOVEMBER,  15)

    testCompression("2014 UR", ur_2014, 0.99, start, end)
  }

  test("2015 QT3 compresses to 34% of original size", RequiresNetwork, Slow) {
  ...
  test("Churyumov-Gerasimenko compresses to 4% of original size", RequiresNetwork, Slow) {
  ...
  test("Titan compresses to 1% of original size", RequiresNetwork, Slow) {
  ...
  test("Beer compresses to 5% of original size", RequiresNetwork, Slow) {
  ...
  test("Io compresses to 24% of original size", RequiresNetwork, Slow) {
  ...
  def testCompression(
    ...
    println(f"$name%-10s $key%-22s => $size%5d / $elems%5d elements, ${ratio * 100.0}%5.2f%% of original")
