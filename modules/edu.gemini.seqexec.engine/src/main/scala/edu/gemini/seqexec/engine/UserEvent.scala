...
final case class GetState[D<:Engine.Types](f: (Engine.State[D#StateData]) => Task[Option[Process[Task, Event[D]]]]) extends UserEvent[D] {
  val user: Option[UserDetails] = None
}
// Generic event to put a function in the main Process process, which takes an
// action depending on the current state of a specific sequence
final case class GetSeqState[D<:Engine.Types](id: Sequence.Id, f: Sequence.State => Option[Process[Task, Event[D]]]) extends UserEvent[D] {
  val user: Option[UserDetails] = None
}
...
final case class ActionStop[D<:Engine.Types](id: Sequence.Id, f: Sequence.State => Option[Process[Task, Event[D]]]) extends UserEvent[D] {
  val user: Option[UserDetails] = None
}
