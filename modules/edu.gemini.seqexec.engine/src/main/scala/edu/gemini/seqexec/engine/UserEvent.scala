...
case object Poll extends UserEvent[Nothing] {
  val user: Option[UserDetails] = None
}
// Generic event to put a function in the main Process process, which takes an
// action depending on the current state
final case class GetState[D](f: (Engine.State[D]) => Task[Option[Process[Task, Event[D]]]]) extends UserEvent[D] {
  val user: Option[UserDetails] = None
}
// Generic event to put a function in the main Process process, which takes an
// action depending on the current state of a specific sequence
final case class GetSeqState[D](id: Sequence.Id, f: Sequence.State => Option[Process[Task, Event[D]]]) extends UserEvent[D] {
  val user: Option[UserDetails] = None
}
// Generic event to put a function in the main Process process, which changes the state
// depending on the current state
final case class ModifyState[D](f: (Engine.State[D]) => Engine.State[D]) extends UserEvent[D] {
  val user: Option[UserDetails] = None
}
...
final case class ActionStop[D](id: Sequence.Id, f: Sequence.State => Option[Process[Task, Event[D]]]) extends UserEvent[D] {
  val user: Option[UserDetails] = None
}

// Uses `cont` to resume execution of a paused Action. If the Action is not paused, it does nothing.
final case class ActionResume(id: Sequence.Id, i: Int, cont: Task[Result]) extends UserEvent[Nothing] {
  val user: Option[UserDetails] = None
}
