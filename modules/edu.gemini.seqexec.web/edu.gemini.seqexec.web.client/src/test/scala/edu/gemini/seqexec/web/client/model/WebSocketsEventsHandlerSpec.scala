...
class WebSocketsEventsHandlerSpec extends FlatSpec with Matchers {
  ...
  "WebSocketsEventsHandler" should "accept connection open events anonymously" in {
    val handler = new WebSocketEventsHandler(zoomRW(m => (m.uiModel.sequences, m.uiModel.user, m.site))((m, v) => m.copy(uiModel = m.uiModel.copy(sequences = v._1, user = v._2), site = v._3)))
