...
class SeqTranslate(site: Site, systems: Systems[IO], settings: TranslateSettings) extends ObserveActions {
  ...
  private def calcSystems(
    ...
    stepType match {
      case StepType.CelestialObject(inst) => getTcs(inst.hasOI.fold(allButGaos, allButGaosNorOi), useGaos = false, sys,
          TcsController.LightSource.Sky, config).map{ List(sys, _, Gcal.defaultGcal(systems.gcal)) }

      case StepType.NodAndShuffle(inst)   => getTcs(inst.hasOI.fold(allButGaos, allButGaosNorOi), useGaos = false, sys,
          TcsController.LightSource.Sky, config).map{ List(sys, _, Gcal.defaultGcal(systems.gcal)) }
      ...
      case StepType.AltairObs(inst)       => getTcs(inst.hasOI.fold(allButGaos, allButGaosNorOi).add(Gaos),
        useGaos = true, sys, TcsController.LightSource.AO, config).map{ List(sys, _, Gcal.defaultGcal(systems.gcal)) }

      case StepType.AlignAndCalib         => List(sys).asRight

      case StepType.Gems(inst)            => getTcs(inst.hasOI.fold(allButGaos, allButGaosNorOi).add(Gaos),
        useGaos = true, sys, TcsController.LightSource.AO, config).map{ List(sys, _, Gcal.defaultGcal(systems.gcal)) }
