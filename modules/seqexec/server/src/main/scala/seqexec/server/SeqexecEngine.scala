...
object SeqexecEngine {
  ...
  private class SeqexecEngineImpl[F[_]: ConcurrentEffect: Timer: Logger](
    ...
    private case class TargetMatchResult(
      startStep:      StepId,
    ...
    private def sequenceTcsTargetMatch(
      obs: SequenceData[F],
      stp: Option[StepId]
    ): F[Option[TargetMatchResult]] =
      (
        for {
          stepId    <- stp.orElse(obs.seq.currentStep.map(_.id))
          a         <-
            obs.seqGen.steps.dropWhile(_.id =!= stepId).find(a => stepRequiresTargetCheck(a.config))
    ...
    private def stepRequiresTargetCheck(config: CleanConfig): Boolean =
      (for {
    ...
    private def startCheckResources(
      startAction: HandleType[F, Unit],
      id:          Observation.Id,
      clientId:    ClientId,
    ...
    override def start(
      ...
      q.enqueue1(
        Event.modifyState[F, EngineState[F], SeqEvent](
          clearObsCmd(id) *> startCheckResources(executeEngine.start(id),
                                                 id,
                                                 clientId,
                                                 none,
                                                 runOverride
    ...
    override def startFrom(
      ...
      q.enqueue1(
        Event.modifyState[F, EngineState[F], SeqEvent](
          clearObsCmd(id) *> startCheckResources(executeEngine.startFrom(id, stp),
                                                 id,
                                                 clientId,
                                                 stp.some,
                                                 runOverride
    ...
    private def selectSequenceEvent(
      ...
      val lens =
        (EngineState.sequences[F] ^|-? index(sid))
          ...
          refreshSequence(sid)
      def testRunning(st: EngineState[F]): Boolean =
        (for {
    ...
    private def heartbeatStream: Stream[F, EventType[F]] = {
      ...
      Stream
        ...
        .through(noHeartbeatDetection.andThen(_.recoverWith {
          case _ =>
    ...
    override def stream(
      p:  Stream[F, EventType[F]]
    )(s0: EngineState[F]): Stream[F, (EventResult[SeqEvent], EngineState[F])] =
    ...
    override def abortObserve(q: EventQueue[F], seqId: Observation.Id): F[Unit] =
      q.enqueue1(
    ...
    private def clearQ(qid: QueueId): Endo[EngineState[F]] =
      st =>
        st.queues
          .get(qid)
    ...
    override def clearQueue(q: EventQueue[F], qid: QueueId): F[Unit] =
      q.enqueue1(
        Event.modifyState[F, EngineState[F], SeqEvent](
            ...
            .map {
              _.status(st) match {
                case BatchExecState.Idle | BatchExecState.Stopping =>
                  ((EngineState.queues[F] ^|-? index(qid) ^|-> ExecutionQueue.cmdState)
                    .set(BatchCommandState.Run(observer, user, clientId)) >>> {
                    (_, ())
                  }).toHandle *>
    ...
    private def updateSequenceEndo(
      ...
      st =>
        executeEngine.update(
          seqId,
          toStepList(obsseq.seqGen, HeaderExtraData(st.conditions, st.operator, obsseq.observer))
        )(st)

    private def refreshSequence(id: Observation.Id): Endo[EngineState[F]] =
      (st: EngineState[F]) => {
    ...
    private def refreshSequences: Endo[EngineState[F]] =
      (st: EngineState[F]) => {
  ...
  private def resourcesReserved[F[_]](st: EngineState[F]): Set[Resource] = {
    def reserved(q: ExecutionQueue): Set[Resource] =
      q.queue
  ...
  def findRunnableObservations[F[_]](qid: QueueId)(st: EngineState[F]): Set[Observation.Id] = {
    ...
    obs
      .foldLeft((used, Set.empty[Observation.Id])) {
        case ((u, a), (oid, res)) =>
  ...
  private def nextRunnableObservations[F[_]](qid: QueueId, freed: Set[Resource])(
    ...
    obs
      .foldLeft((used ++ resFailed, Set[Observation.Id]())) {
        case ((u, a), (oid, res)) =>
  ...
  private def viewSequence[F[_]](obsSeq: SequenceData[F]): SequenceView = {
    ...
    def resources(s:     SequenceGen.StepGen[F]): List[Resource] =
      s match {
