...
import edu.gemini.spModel.obscomp.InstConstants.{ARC_OBSERVE_TYPE, DARK_OBSERVE_TYPE, OBSERVE_TYPE_PROP, OBS_CLASS_PROP, SCIENCE_OBSERVE_TYPE}
...
import seqexec.model.{Conditions, Observer, Operator, SequenceState, StepId, StepState, UserDetails}
import seqexec.model.enum._
import seqexec.model.enum.Resource.TCS
import monocle.Monocle._
import org.scalatest.flatspec.AnyFlatSpec
import seqexec.model.dhs.DataId
import seqexec.server.tcs.{DummyTargetKeywordsReader, DummyTcsKeywordsReader, TargetKeywordsReader}
...
class SeqexecEngineSpec extends AnyFlatSpec with Matchers with NonImplicitAssertions {
  ...
  private def testSequence(targetName: String, startStepId: StepId, obsClass: List[ObsClass], obsType: List[String])
  : SequenceGen[IO] = {
  ...
  private def simpleSequenceWithTargetName(name: String): SequenceGen[IO] =
    testSequence(name, 1, List(ObsClass.SCIENCE), List(SCIENCE_OBSERVE_TYPE))

  private def systemsWithTargetName(name: String): Systems[IO] =
    defaultSystems.copy(tcsKeywordReader = new DummyTcsKeywordsReader.DummyTcsKeywordReaderImpl[IO] {
  ...
  "SeqexecEngine start" should "start the sequence if it passes the target check" in {
    ...
    val s0 = ODBSequencesLoader.loadSequenceEndo[IO](seqObsId1, seq, executeEngine).apply(EngineState.default[IO])
    ...
    (for {
      ...
      q <- Queue.bounded[IO, executeEngine.EventType](10)
      sf <- advanceOne(q, s0, seqexecEngine.start(q, seqObsId1, UserDetails("", ""), clientId, RunOverride.Default))
    } yield {
      inside(sf.flatMap(EngineState.sequenceStateIndex[IO](seqObsId1).getOption).map(_.status)) {
        case Some(status) => assert(status.isRunning)
      }
    }).unsafeRunSync()
  }
  ...
  it should "not start the sequence if it fails the target check for science observations" in {
    ...
    val seq = testSequence("proof", 1, List(ObsClass.SCIENCE), List(SCIENCE_OBSERVE_TYPE))
  ...
  it should "not start the sequence if it fails the target check for night calibrations" in {
    ...
    val seq = testSequence("proof", 1, List(ObsClass.PROG_CAL), List(SCIENCE_OBSERVE_TYPE))
  ...
  it should "not start the sequence if it fails the target check for partner calibrations" in {
    ...
    val seq = testSequence("proof", 1, List(ObsClass.PARTNER_CAL), List(SCIENCE_OBSERVE_TYPE))
  ...
  it should "pass the target check for ephemeris target" in {
    ...
    val seq = testSequence("proof.eph", 1, List(ObsClass.SCIENCE), List(SCIENCE_OBSERVE_TYPE))
  ...
  it should "pass the target check for solar objects" in {
    ...
    val seq = testSequence("599", 1, List(ObsClass.SCIENCE), List(SCIENCE_OBSERVE_TYPE))
  ...
  it should "start sequence that fails target check if forced" in {
    ...
    (for {
      ...
      seqexecEngine <- SeqexecEngine.build(Site.GS, systems, defaultSettings, sm)
      q <- Queue.bounded[IO, executeEngine.EventType](10)
      sf <- advanceOne(q, s0, seqexecEngine.start(q, seqObsId1, UserDetails("", ""), clientId, RunOverride.Override))
    } yield {
      inside(sf.flatMap(EngineState.sequenceStateIndex[IO](seqObsId1).getOption).map(_.status)) {
        case Some(status) => assert(status.isRunning)
      }
  ...
  "SeqexecEngine startFrom" should "start the sequence if it passes the target check" in {
    ...
    val seq = testSequence("proof", 1, List(ObsClass.ACQ, ObsClass.SCIENCE), List(ARC_OBSERVE_TYPE, SCIENCE_OBSERVE_TYPE))
    ...
    (for {
      ...
      seqexecEngine <- SeqexecEngine.build(Site.GS, systems, defaultSettings, sm)
      q <- Queue.bounded[IO, executeEngine.EventType](10)
      sf <- advanceOne(q, s0, seqexecEngine.startFrom(q, seqObsId1, 2, clientId, RunOverride.Default))
    } yield {
      inside(sf.flatMap(EngineState.sequenceStateIndex[IO](seqObsId1).getOption).map(_.status)) {
        case Some(status) => assert(status.isRunning)
      }
  ...
  it should "start the sequence tha fails target check if forced" in {
    val systems = systemsWithTargetName("other")
    ...
    (for {
      ...
      seqexecEngine <- SeqexecEngine.build(Site.GS, systems, defaultSettings, sm)
      q <- Queue.bounded[IO, executeEngine.EventType](10)
      sf <- advanceOne(q, s0, seqexecEngine.startFrom(q, seqObsId1, 2, clientId, RunOverride.Override))
    } yield {
      inside(sf.flatMap(EngineState.sequenceStateIndex[IO](seqObsId1).getOption).map(_.status)) {
        case Some(status) => assert(status.isRunning)
      }
  ...
  "SeqexecEngine startFrom" should "not check target for calibrations" in {
    ...
    val seq = testSequence("proof", 1, List(ObsClass.DAY_CAL, ObsClass.DAY_CAL),
