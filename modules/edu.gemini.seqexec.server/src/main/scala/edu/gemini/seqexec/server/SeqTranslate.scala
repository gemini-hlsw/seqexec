...
class SeqTranslate(site: Site, systems: Systems, settings: Settings) {
  ...
  private def deliverObserveCmd(seqState: Sequence.State,
                                f: ObserveControl => Option[SeqAction[Unit]]): Option[Process[Task, EngineEvent]] = {
  ...
  private def handleError(t: TrySeq[Unit]): EngineEvent = t match {
    case -\/(e) => Event.logErrorMsg(SeqexecFailure.explain(e))
    case _      => Event.nullEvent
  }

  def stopObserve(seqId: Sequence.Id)(seqState: Sequence.State): Option[Process[Task, EngineEvent]] = {
  ...
  def abortObserve(seqId: Sequence.Id)(seqState: Sequence.State): Option[Process[Task, EngineEvent]] = {
    def f(oc: ObserveControl): Option[SeqAction[Unit]] = oc match {
  ...
  def pauseObserve(seqState: Sequence.State): Option[Process[Task, EngineEvent]] = {
    def f(oc: ObserveControl): Option[SeqAction[Unit]] = oc match {
  ...
  private def pausedCommand(seqId: Sequence.Id, f: ObserveControl => Option[Time => SeqAction[ObserveCommand.Result]])
                           (seqState: Sequence.State): Option[Process[Task, EngineEvent]] = {
  ...
  def resumePaused(seqId: Sequence.Id)(seqState: Sequence.State): Option[Process[Task, EngineEvent]] = {
  ...
  private def stopPaused(seqId: Sequence.Id)(seqState: Sequence.State): Option[Process[Task, EngineEvent]] = {
    def f(o: ObserveControl): Option[Time => SeqAction[ObserveCommand.Result]] = o match {
  ...
  private def abortPaused(seqId: Sequence.Id)(seqState: Sequence.State): Option[Process[Task, EngineEvent]] = {
    def f(o: ObserveControl): Option[Time => SeqAction[ObserveCommand.Result]] = o match {
