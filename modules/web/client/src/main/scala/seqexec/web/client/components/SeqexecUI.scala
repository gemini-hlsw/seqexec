 ...
 */
object SeqexecUI {
  private def pageTitle(site: Site)(p: SeqexecPages): String =
    p match {
  ...
  def router[F[_]](site: Site)(implicit F: Sync[F]): F[Router[SeqexecPages]] = {
    ...
    val routerConfig = RouterConfigDsl[SeqexecPages].buildConfig { dsl =>
      ...
      (emptyRule
        ...
        ) ~> dynRenderR((_: SequenceConfigPage, r) => SeqexecMain(site, r))
        | dynamicRouteCT(
          ("/" ~ string("[a-zA-Z0-9-]+") ~ "/" ~ string("[a-zA-Z0-9-]+") ~ ("/step/" ~ int).option)
            .pmapL(sequencePageSP(instrumentNames))
        ) ~> dynRenderR((_: SequencePage, r) => SeqexecMain(site, r))
        | dynamicRouteCT(
          ("/preview/" ~ string("[a-zA-Z0-9-]+") ~ "/" ~ string(
            "[a-zA-Z0-9-]+"
          ) ~ ("/step/" ~ int).option)
            .pmapL(previewPageSP(instrumentNames))
        ) ~> dynRenderR((_: PreviewPage, r) => SeqexecMain(site, r))
        | dynamicRouteCT(
          ("/preview/" ~ string("[a-zA-Z0-9-]+") ~ "/" ~ string(
            "[a-zA-Z0-9-]+"
          ) ~ "/configuration/" ~ int)
            .pmapL(previewConfigPageP(instrumentNames))
        ) ~> dynRenderR((_: PreviewConfigPage, r) => SeqexecMain(site, r)))
        ...
        .onPostRender((_, next) =>
          Callback.when(next === SoundTest)(SeqexecCircuit.dispatchCB(RequestSoundEcho)) *>
            Callback.when(next =!= SeqexecCircuit.zoom(_.uiModel.navLocation).value)(
              SeqexecCircuit.dispatchCB(NavigateSilentTo(next))
            )
        )
    ...
    def navigated(
      routerLogic: RouterLogic[SeqexecPages, Unit],
      page:        ModelRO[SeqexecPages]
    ): SetTimeoutHandle =
    ...
    for {
      ...
      _                    <- F.delay(SeqexecCircuit.subscribe(SeqexecCircuit.zoom(_.uiModel.navLocation)) { x =>
