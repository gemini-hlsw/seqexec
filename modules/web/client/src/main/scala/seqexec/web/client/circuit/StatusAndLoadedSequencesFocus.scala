...
object StatusAndLoadedSequencesFocus {
  ...
  val statusAndLoadedSequencesG: Getter[SeqexecAppRootModel, StatusAndLoadedSequencesFocus] =
    ClientStatus.clientStatusFocusL.asGetter.zip(
      sessionQueueG.zip(
        sodG.zip(
          SeqexecAppRootModel.sessionQueueTableStateL.asGetter
  ...
  val filteredSequencesG: Getter[SeqexecAppRootModel, List[SequenceInSessionQueue]] = {
    sessionQueueFilterG.zip(sessionQueueG.zip(sodG.zip(SeqexecAppRootModel.dayCalG))) >>> {
      case (f, (s, (sod, dayCal))) =>
        f.filter(
          SequenceInSessionQueue
